...
A label selector is evaluated at a single point in time and matches only the
`Application`s that carry the matching labels **at that moment.** When the
matching `Application`s are produced by a generator such as an `ApplicationSet`,
they may not all exist — or be labeled — at once. If the step observes only an
early subset and finds it healthy and synced, it can succeed prematurely, before
the remaining `Application`s have even been created. Use `minMatches` to require
a minimum number of matches before readiness is evaluated, and `desiredRevisions`
to require that matched `Application`s are synced to a specific revision rather
than trusting a possibly stale `Synced` status. See
[Waiting on Applications Managed by an ApplicationSet](#waiting-on-applications-managed-by-an-applicationset).
...
| `apps[].minMatches` | `integer` | N | The minimum number of `Application`s that must match the `selector` before their readiness is evaluated. Until at least this many match, the step keeps waiting. This guards against a race in which a generator (such as an `ApplicationSet`) has not yet produced or labeled all expected `Application`s, causing the step to succeed after observing only an early subset. Ignored when `name` is used. Defaults to `0`, in which case at least one match is required. |
| `apps[].desiredRevisions` | `[]string` | N | An optional list of source revisions (e.g. Git commit SHAs or chart versions) that each matched `Application` must be observably synced to before it is considered ready. Each specified revision must appear among an `Application`'s synced source revisions. This guards against trusting a stale `Synced` status left over from a previous revision. Typically set using an expression such as `${{ commitFrom(...).ID }}`. |
...
### Waiting on Applications Managed by an ApplicationSet

When the `Application`s you are waiting on are generated by an `ApplicationSet`,
the generator labels and creates them progressively after the underlying
configuration changes. A bare label selector can match an early subset and
succeed before the rest exist. Combine `minMatches` with `desiredRevisions` to
wait for the full set to converge on the expected revision.

In this example, the promotion has pushed a new commit and expects the
`ApplicationSet` to produce 13 production `Application`s, all synced to that
commit, before the `Promotion` proceeds.

```yaml
steps:
# Update instance files, commit, push, etc...
- uses: argocd-wait
  config:
    apps:
    - selector:
      ...
      minMatches: 13
      desiredRevisions:
      - ${{ commitFrom("https://github.com/example/repo").ID }}
```
